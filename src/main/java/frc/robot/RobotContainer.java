...
public class RobotContainer {
        ...
        private final SwerveRequest.FieldCentric drive = new SwerveRequest.FieldCentric()
                        .withDeadband(MaxSpeed * 0.02).withRotationalDeadband(MaxAngularRate * 0.02) // Add a 10%
                                                                                                     // deadband
        ...
        public RobotContainer() {
                ...
                autoDirector = new AutoDirector(
                                new AutoSubsystems(m_DriveSubsystem, m_ArmSubsystem, m_ElevatorSubsystem,
                                                m_GrabberSubsystem, m_Intake, algaeCam, m_IntakeSubsystem));
                Pose2d robotPose = m_DriveSubsystem.getState().Pose;
                Pose2d questPose = robotPose.transformBy(Constants.Inputs.Quest.Robot_To_Quest);
        ...
        private void configureBindings() {
                ...
                Driver.autoAlignLeft().whileTrue(
                                new DeferredCommand(() -> m_DriveSubsystem.Commands.autoAlignV2("B"),
                                                Set.of(m_DriveSubsystem)));
                Driver.autoAlignRight().whileTrue(
                                new DeferredCommand(() -> m_DriveSubsystem.Commands.autoAlignV2("A"),
                                                Set.of(m_DriveSubsystem)));
                Driver.outTake()
                                .whileTrue(m_GrabberSubsystem.Commands.outtake()
                                                .alongWith(m_ElevatorSubsystem.Commands.outtake()));
                Driver.outTake()
                                .and(m_GrabberSubsystem.Commands.checkCoralReversed())
                                .onTrue(Commands.runOnce(() -> SignalLogger.writeString("Scored Coral Pose",
                                                m_DriveSubsystem.getState().Pose.toString())));

                Driver.superStructureL1()
                                .onTrue(m_ElevatorSubsystem.Commands.L1().alongWith(m_ArmSubsystem.Commands.L1()));
                Driver.superStructureL2()
                                .onTrue(m_ElevatorSubsystem.Commands.L2().alongWith(m_ArmSubsystem.Commands.L2()));
                Driver.superStructureL3()
                                .onTrue(m_ElevatorSubsystem.Commands.L3().alongWith(m_ArmSubsystem.Commands.L3()));
                Driver.superStructureL4()
                                .onTrue(m_ElevatorSubsystem.Commands.L4().alongWith(m_ArmSubsystem.Commands.L4()));
                ...
                Driver.verticalCoralIntake().whileTrue(m_GrabberSubsystem.Commands.intake()
                                .alongWith(m_ArmSubsystem.Commands.GroundIntake())
                                .alongWith(m_ElevatorSubsystem.Commands.Bottom()));
                Driver.bellyPanIntake()
                                .whileTrue((m_ElevatorSubsystem.Commands.intakePrime()
                                                .andThen(Commands.waitUntil(() -> m_ElevatorSubsystem.atSetpoint())))
                ...
                Driver.bellyPanIntake().and(Driver.Prime()).whileTrue(
                                new intake(m_ElevatorSubsystem, m_ArmSubsystem, m_GrabberSubsystem, m_IntakeSubsystem));

                Driver.verticalCoralIntake().and(Driver.Prime())
                                .onTrue(m_ElevatorSubsystem.Commands.Bottom()
                                                .alongWith(m_ArmSubsystem.Commands.VerticalStow()));
                Driver.manUpElev().whileTrue(m_ElevatorSubsystem.Commands.changeSetpointBy(Inches.of(.5)).repeatedly());
                Driver.manDownElev()
                                .whileTrue(m_ElevatorSubsystem.Commands.changeSetpointBy(Inches.of(-.5)).repeatedly());
                ...
                SmartDashboard.putData("Coral In the Bot", m_ElevatorSubsystem.Commands.CoralStuck()
                                .alongWith(m_ArmSubsystem.Commands.CoralStuck()));
                // SYSID ROUTINES
        ...
        public void updateValues() {
                ...
                // Comment out this line if field relitive becomes an issue.
                ...
                SmartDashboard.putNumberArray("Speeds {X,Y,Theta}",
                                new double[] { m_DriveSubsystem.getState().Speeds.vxMetersPerSecond,
                                                m_DriveSubsystem.getState().Speeds.vyMetersPerSecond,
                                                m_DriveSubsystem.getState().Speeds.omegaRadiansPerSecond });
        }
        ...
        public void feedQuestNav() {
                if (quest.isConnected() && quest.isTracking()) {
                        Pose2d questPose = quest.getPose();
                        Pose2d robotPose = questPose.transformBy(Constants.Inputs.Quest.Robot_To_Quest.inverse());
                        double timestamp = quest.getDataTimestamp();
                        m_DriveSubsystem.addVisionMeasurement(robotPose, timestamp, Constants.Inputs.Quest.QUESTNAV_STD_DEVS);
                }
        ...
        public void feedVision(Vision vision) {
                if (vision.getCameraPipeline() == CameraPipeline.AprilTag) {
                        ...
                        if (visionEst != null) {
                                visionEst.ifPresent(
                                                est -> {
                                                        // Change our trust in the measurement based on the tags we can
                                                        // see
